const STATE={origin:null,stops:[],run:0,repairing:false};
...
function waitForReady(){const token=++STATE.run;let tries=0;const timer=setInterval(async()=>{if(token!==STATE.run){clearInterval(timer);return}tries++;if($('status')?.textContent==='Rota pronta'){clearInterval(timer);await repairHairpins()}else if(tries>120)clearInterval(timer)},250)}
